...
const Display = ({ input, result, history }) => (
  <div className="relative p-4 mb-2 text-2xl text-right text-white bg-gray-700 rounded">
    ...
    <div className={`h-24 pr-4 ${history.length > 3 ? 'overflow-y-auto scrollbar-thumb-gray-500 scrollbar-track-gray-700 scrollbar-thin' : ''}`}>
      {history.slice(-3).map((entry, index) => (
        <div key={index} className="text-gray-300">{entry.input} = {entry.result}</div>
      ))}
    </div>
    <div>{input}</div>
    ...
    <div className="h-screen w-screen flex justify-center items-center bg-gray-100 ">
      <div className={`bg-gray-100 p-8 rounded-lg shadow-lg max-w-2xl mx-auto h-full ${theme === 'dark'? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
        ...
        <div className="bg-gray-900 p-4 rounded-lg shadow-lg max-w-2xl mx-auto h-full">
          <Display input={input} result={result} history={history} />
